/**
 * app/(routes)/confirm/page.tsx
 * ──────────────────────────────────────────────────────────────
 * The page users land on when they click the confirmation link in
 * the subscription email. The link looks like:
 *   https://yourchurch.org/confirm?token=abc123...
 * ──────────────────────────────────────────────────────────────
 */
...
export default function ConfirmPage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      setStatus("error");
      setMessage("No confirmation token found. This link may be invalid.");
      return;
    }

    fetch(`/api/newsletter/confirm/${encodeURIComponent(token)}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.ok) {
          setStatus("success");
  ...
  return (
    <>
      <BodyClass className="confirm-page" />
