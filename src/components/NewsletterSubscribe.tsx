      ...
      {status === "success" ? (
        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2">
          <input
            ...
            disabled={status === "loading"}
            className="px-4 py-2.5 rounded-lg bg-white/10 border border-white/10 text-white placeholder:text-white/40 text-sm outline-none focus:border-brand-accent transition-colors disabled:opacity-50 flex-1 min-w-[200px]"
          />
          <button
            type="submit"
            disabled={status === "loading"}
            className="px-6 py-2.5 rounded-lg bg-brand-accent text-brand-primary font-bold text-sm hover:brightness-110 transition-all disabled:opacity-50 whitespace-nowrap"
          >
